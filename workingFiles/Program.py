...
class TestApp(TestWrapper, TestClient):
    ...
    def tickPrice(self, reqId: TickerId, tickType: TickType, price: float,
        ...
        if len(self.priceList) == 3:
            self.placeOrder(self.nextOrderId(), Contracts.AnyStock(self.stock),
                            Orders.NewTrailingStop("SELL", self.amount, self.trail, 0))
            self.nextSellingPrice = self.price - self.trail
        # todo turn thw first two orders to a bracket order
        if self.pos == self.amount / 2:  # long position
            ...
            if self.filled == self.amount and self.bought == False:
                self.placeOrder(self.nextOrderId(), Contracts.AnyStock(self.stock),
                                Orders.NewTrailingStop("BUY", self.amount, self.trail, 0))
                self.sold = False
                self.bought = True
                self.filled = 0
        # if self.trail == 0.01:
        #     try:
        #         if self.pos == self.amount/2:
        #             if self.price > self.priceList[-1]:
        #                 print("up ", self.price)
        #             elif self.price < self.priceList[-1] and self.sold == False and self.filled == self.amount:
        ...
        #                 pass
        #
        #         elif self.pos == -1*self.amount/2:
        #             if self.price > self.priceList[-1] and self.bought == False and self.filled == self.amount:
        #                 print('^^^^^^^^^^^^^^buy^{}^^^^^^^^^^^^^^^^'.format(self.price))
        #                 self.placeOrder(self.nextOrderId(), Contracts.AnyStock(self.stock),
        #                                 Orders.NewTrailingStop("BUY", self.amount, self.trail, 0))
        #                 self.bought = True
        #                 self.sold = False
        #                 self.filled = 0
        #             elif self.price < self.priceList[-1]:
        #                 print("down ", self.price)
        #             else:
        #                 pass
        #         else:
        #             pass
        ...
        #             self.bought = False
        #             self.filled = 0
        #         # print('next price: ', self.nextSellingPrice)
        #         # gap = self.nextSellingPrice - self.price
        #         # print('gap: ', gap)
        #         # if gap <= 0:  # new price is bigger then last price
        #         #     print("up ", self.price)
        #         # else:
        #         #     if gap == self.trail and self.sold == False and self.filled == self.amount:
        #         #         print('mky price - {} tick price - {}'.format(mktCapPrice, self.price))
        #         #         # assert(mktCapPrice == self.price)
        #         #         print('^^^^^^^^^^^^^^sell^{}^^^^^^^^^^^^^^^^'.format(self.price))
        #         #         self.placeOrder(self.nextOrderId(), Contracts.AnyStock(self.stock),
        #         #                         Orders.NewTrailingStop("SELL", self.amount, self.trail, 0))
        #         #         self.sold = True
        ...
        #         #         self.filled = 0
        #         #         self.nextSellingPrice = self.price - self.trail
        #         #     else:  # gap in less then trail amount
        #         #         pass
        #
        #     elif self.pos == -1*self.amount/2:  # short position
        #         if self.filled == self.amount and self.bought == False:
        ...
        #             self.filled = 0
        #
        #         # gap = self.price - self.nextSellingPrice
        ...
        #         #     pass
        #         # else:
        #         #     if gap >= self.trail and self.bought == False and self.filled == self.amount:
        #         #         print('mky price - {} tick price - {}'.format(mktCapPrice, self.price))
        #         #         # assert(mktCapPrice == self.price)
        #         #         print('^^^^^^^^^^^^^^buy^{}^^^^^^^^^^^^^^^^'.format(self.price))
        #         #         self.placeOrder(self.nextOrderId(), Contracts.AnyStock(self.stock),
    ...
    #                 self.stock_dir = 'up'
    #                 if len(self.priceList) == 3:
    #                     self.bracketSample('BUY', 100, self.price, trail)
    #             else:
    #                 self.stock_dir = 'down'
    #                 if len(self.priceList) == 3:
    #                     self.bracketSample('SELL', 100, self.price, trail)
    #         except:
    #             pass
    #     else:
    #         # print("stock direction: ", self.stock_dir)
    #         # print("Pricelist: ", self.priceList)
    #         # print("Price: ", price)
    #         try:
    #             if self.stock_dir == 'up':
    #                 if self.price > self.priceList[-1]:
    #                     if self.pos == 'short':
    #                         pass
    #                     else:
    #                         self.trailStopPrice = self.price - trail
    #                         # print('^^^^^^^^^^^^^^trailStopPrice {}^^^^^^^^^^^^^^^^^^^'.format(self.trailStopPrice))
    #                 elif self.price < self.priceList[-1]:
    #                     # print('^^^^^^^^^^^^^^turn over^^^^^^^^^^^^^^^^^')
    #                     self.stock_dir = 'down'
    #                     print('^^^^^^^^^^^^^^trailStopPrice {}^^^^^^^^^^^^^^^^^^^'.format(self.trailStopPrice))
    #                     # if self.price == self.trailStopPrice:
    #                     self.bracketSample('SELL', 200, self.price, trail)
    #                     # self.pos = 'short'
    #                     # print('sell 200')
    #                     # self.trailStopPrice = self.price + trail
    ...
    #                     # self.stock_dir = 'up'
    #                     # if self.price == self.trailStopPrice:
    #                     self.bracketSample('BUY', 200, self.price, trail)
    #                     # self.pos = 'long'
    #                     # print('buy 200')
    #                     # self.placeOrder(self.nextOrderId(), Contracts.TQQQstock(),
    #                     #                 Orders.NewTrailingStop("BUY", 200, 0.01, 0))
    #                     # self.trailStopPrice = self.price - trail
    #                 elif self.price < self.priceList[-1]:
    #                     if self.pos == 'long':
    #                         pass
    ...
    #             pass
    #             # self.placeOrder(self.nextOrderId(), Contracts.TQQQstock(),
    #             #                 Orders.NewTrailingStop("BUY", 100, 0.01, 0))
    #     self.priceList.append(price)
    #
